...
class LSTM_TF_DeCov_MLP_init_cell(LSTM_TF_DeCov):
    ...
    def prediction(self):
        ...
        self.first_cell_init_c_1 = tf.layers.dense(inputs=self.input_tensor[:, 0, :5], units=64, activation=tf.nn.relu)
        self.first_cell_init_c_2 = tf.layers.dense(inputs=self.first_cell_init_c_1, units=64, activation=tf.nn.relu)

        self.deep_initial_state = tf.cond(
            tf.equal(tf.reduce_sum(tf.abs(self.input_tensor[:, 0, :5])), 0.0),
            lambda: self.lstm_initial_state,
            lambda: (tf.nn.rnn_cell.LSTMStateTuple(self.first_cell_init_c_2,
                                                   self.lstm_initial_state[0].h),
