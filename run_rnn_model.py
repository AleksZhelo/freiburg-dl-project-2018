...
def run_rnn_model(session, configs, learning_curves, log_dir,
                  ...
                  tf_seed=1123, numpy_seed=1123, verbose=True, reload_existing=False,
                  post_fold_func=None):
    ...
    for train_indices, test_indices in k_fold.split(configs):
        ...
        fold_save_path = os.path.join(save_dir, '{0}_fold_{1}.ckpt'.format(model_desc, current_fold))
        total_epochs = 0
        curr_steps = 0
        if reload_existing and os.path.exists(fold_save_path):
            saver.restore(session, fold_save_path)
        else:
            while total_epochs < int(np.round(train_epochs)):
                ...
                if total_epochs % eval_every == 0:
                    ...
                    if early_stopping:
                        valid_losses, valid_pred = loss_and_predictions_for_several_n_input(
                            initial_state, n_test, 40,
                            phase, rnn, session,
                            valid_configs, valid_curves
                        )
                        valid_mse = valid_losses.mean()
                        if verbose:
                        ...
                        if valid_mse < best_valid:
                            ...
                            if save_dir is not None:
                                if verbose:
                                    print('saved model')
                                saver.save(session, fold_save_path)
                        else:
                            counter += 1
                            if counter > patience:
                                if verbose:
        ...
        performances_test[current_fold, :], predictions_test = \
            loss_and_predictions_for_several_n_input(
                initial_state, [5, 10, 20, 30], 40,
                phase, rnn, session,
                test_configs, test_curves
            )
        if early_stopping:
            performances_valid[current_fold, :], predictions_valid = \
                loss_and_predictions_for_several_n_input(
                    initial_state, [5, 10, 20, 30], 40,
                    phase, rnn, session,
                    valid_configs, valid_curves
        ...
        if post_fold_func is not None:
            post_fold_func(predictions_test,
...
def loss_and_predictions_for_several_n_input(initial_state, n_input, n_output,
                                             phase, rnn, session, configs, curves):
    if not isinstance(n_input, list):
        n_input = [n_input]
...
if __name__ == '__main__':
    ...
    with tf.Session() as session:
        params = {'learning_rate': 0.00350212567264791, 'reg_weight': 2.063759204812831e-05}
        # params = {
        #     'learning_rate': 0.005,
        #     'reg_weight': 0.1,
        # # 'drop_rate': 0.00,
