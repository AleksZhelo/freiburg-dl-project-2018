...
def get_random_config():
    return {"lr" : np.power(10.0, np.random.uniform(-4, -2)),
            "batchsize" : 1, #int(np.power(2.0, np.random.uniform(0, 6))),
...
def task3(return_dict,
    ...
    def predict_whole_sequences(model, X):
        ...
        Y_hat = np.zeros(shape=(n, final_step - 1, 1))
        for i in range(n):
            pred = model.predict(X[i:(i+1),:,:])
            Y_hat[i, :true_steps, 0] = pred[:, 0]
            for j in range(true_steps, final_step - 1):
                x = np.zeros(shape=(1,1,d))
                x[:,:,-1] = pred[0, -1]
                if repeat_config:
                    x[:,:,:-1] = X[0,0,:-1]
                pred = model.predict(x, batch_size=1)
                Y_hat[i:(i+1), j, 0] = pred
            model.reset_states()
        return Y_hat
    ...
    for training_indices, test_indices in k_fold.split(learning_curves):
        ...
        n_batches = int(np.floor(n_train / batchsize))
        ...
        for epoch in range(epochs):
            ...
            for batch in range(n_batches):
                ...
                y_hat = model.predict(x, batch_size=x.shape[0])
                model.reset_states()
            ...
            # validation
            if (epoch + 1) % 1 == 0:
                Y_hat = np.zeros(shape=(n_valid, n_steps_valid))
                for j in range(n_valid):
                    Y_hat[j:(j+1),:] = model.predict(X_valid[j:(j+1), :n_steps_valid, :], batch_size=1)[:, :, 0]
                    model.reset_states()
